...
const groupsbyName = books.reduce(
  (acc, curr) => {
    curr.genre === "Fiction"
      ? acc["fiction"].push(curr.title)
      : acc["non-fiction"].push(curr.title);
    return acc;
  },
  { fiction: [], "non-fiction": [] }
);
...
let merged = ordersList.map((order) => {
  const customer = customers.find((c) => order.customerId == c.id);
  return { ...order, name: customer.name || null };
});
console.log(merged);
